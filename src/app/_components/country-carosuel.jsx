...
const CountryCard = ({ country, isExpanded, onMouseEnter }) => {
  return (
    ...
    >
      ...
      />
      <div className="absolute left-4 top-4 rounded-full bg-white/90 p-2 text-2xl">
        {country.flag}
      </div>
      <AnimatePresence>
        {isExpanded && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-[#87BD2B] to-[#87BD2B]/80 p-6 text-white w-[90%] rounded-2xl mx-auto mb-2"
          >
            <h3 className="text-2xl font-bold text-white">{country.name}</h3>
            <p className="mt-2 text-white">{country.description}</p>
            <motion.button 
